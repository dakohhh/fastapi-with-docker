...
class Auth:
    ...
    async def authenticate_user(self, login_input: LoginSchema):
        user = await UserRepository.get_user_by_email(email=login_input.email)
    ...
    async def get_current_user(
        ...
        user = await UserRepository.get_user_by_id(access_token_data.user)

        if user is None:
            raise CredentialsException("this User doesn't exists")
